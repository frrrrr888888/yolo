...
def get_cfg(cfg: str | Path | dict | SimpleNamespace = DEFAULT_CFG_DICT, overrides: dict = None) -> SimpleNamespace:
    ...
    Examples:
        ...
        >>> config = get_cfg()  # Load config configuration
...
def get_save_dir(args: SimpleNamespace, name: str = None) -> Path:
    """
    Return the directory path for saving outputs, derived from arguments or config settings.
    ...
    if getattr(args, "save_dir", None):
        ...
        if getattr(args, "save_dir_name", None):
            project = args.project or (save_dir / f"tests/tmp/{args.save_dir_name}" if TESTS_RUNNING else RUNS_DIR) / args.task
...
def handle_yolo_settings(args: list[str]) -> None:
    ...
    Notes:
        ...
        - The 'reset' command will delete the existing settings file and create new config settings.
...
def handle_yolo_solutions(args: list[str]) -> None:
    ...
    Examples:
        Run people counting solution with config settings:
    ...
    Notes:
        ...
        - If no source is specified, downloads and uses a config sample video
...
def entrypoint(debug: str = "") -> None:
    ...
    for a in merge_equals_args(args):  # merge spaces around '=' sign
        ...
        if "=" in a:
            ...
            overrides[a] = True  # auto-True for config bool args, i.e. 'yolo show' sets show=True
    ...
    if mode is None:
        ...
        LOGGER.warning(f"'mode' argument is missing. Valid modes are {list(MODES)}. Using config 'mode={mode}'.")
    ...
    if model is None:
        model = "yolo11n.pt"
        LOGGER.warning(f"'model' argument is missing. Using config 'model={model}'.")
    ...
    if mode in {"predict", "track"} and "source" not in overrides:
        ...
        LOGGER.warning(f"'source' argument is missing. Using config 'source={overrides['source']}'.")
    elif mode in {"train", "val"}:
        if "data" not in overrides and "resume" not in overrides:
            overrides["data"] = DEFAULT_CFG.data or TASK2DATA.get(task or DEFAULT_CFG.task, DEFAULT_CFG.data)
            LOGGER.warning(f"'data' argument is missing. Using config 'data={overrides['data']}'.")
    elif mode == "export":
        if "format" not in overrides:
            overrides["format"] = DEFAULT_CFG.format or "torchscript"
            LOGGER.warning(f"'format' argument is missing. Using config 'format={overrides['format']}'.")

    # Run command in python
    getattr(model, mode)(**overrides)  # config args from model
...
def copy_default_cfg() -> None:
    """
    Copy the config configuration file and create a new one with '_copy' appended to its name.

    This function duplicates the existing config configuration file (DEFAULT_CFG_PATH) and saves it
    with '_copy' appended to its name in the current working directory. It provides a convenient way
    to create a custom configuration file based on the config settings.

    Examples:
        >>> copy_default_cfg()
        # Output: config.yaml copied to /path/to/current/directory/default_copy.yaml
    ...
    Notes:
        ...
        - This function is useful for users who want to modify the config configuration without
