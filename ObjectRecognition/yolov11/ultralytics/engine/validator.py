...
class BaseValidator:
    ...
    def __init__(self, dataloader=None, save_dir=None, args=None, _callbacks=None, cfg_path=None):
        """
        ...
        if cfg_path:
            self.args = get_cfg(cfg=cfg_path, overrides=args)
        else:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        # 创建txt结果文档存储文件夹
        results_txt_dir = os.path.join(self.save_dir, "results_txt")
        ...
        for batch_i, batch in enumerate(bar):
            ...
            # 将文件名转为txt路径（内置参数可以存）
            # txt_files = [os.path.join(results_txt_dir, os.path.splitext(os.path.basename(im_file))[0] + ".txt")
            ...
            with dt[1]:
                if self.args.visualize_feature:
                    preds = model(batch["img"], augment=augment, visualize=self.save_dir)
                else:
                    tic = time.time()
            ...
            # Postprocess
            with dt[3]:
                preds = self.postprocess(preds)  # preds[0].keys(): dict_keys(['bboxes', 'conf', 'cls', 'keypoints'])
        ...
        print(f"{np.mean(np.array(time_list))}ms")
